						...
						<div class="widget-body">
							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								...
								<tbody>
									...
									<tr data-id="<?= $q['paynt_id'] ?>">
										<td><?=$q['paynt_name']?></td>
										<td><?=$q['paynt_description']?></td>
										<td><?=$q['paynt_status']?></td>
										<td>
											<button class="btn btn-primary btn-sm" onclick="Update(<?= $q['paynt_id'] ?>, '<?= $q['paynt_name'] ?>', '<?= $q['paynt_description'] ?>', '<?= $q['paynt_status'] ?>')">
    ...
    $(() =>
    {
	    ...
	    $('#form').submit(ev =>
	    {
	    	...
	    	if(type == 'ADD')
	    	{
	    		...
	    		$.ajax({
	    			method: 'post',
	    			url: '<?= base_url ?>admin/sales-payment-method-create',
	    			data: {
	    				paynt_name: name,
	    				paynt_description: description,
	    				paynt_status: status,
	    			}
	    		})
	    	...
	    	else
	    	{
	    		...
	    		$.ajax({
	    			method: 'put',
	    			url: '<?= base_url ?>admin/sales-payment-method-update',
	    			data: {
	    				paynt_id: id,
	    				paynt_name: name,
	    				paynt_description: description,
