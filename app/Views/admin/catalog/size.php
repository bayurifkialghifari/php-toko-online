						...
						<div class="widget-body">
							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								...
								<tbody>
									...
									<tr data-id="<?= $q['size_id'] ?>">
										<td><?=$q['cate_name']?></td>
										<td><?=$q['size_name']?></td>
										<td><?=$q['size_description']?></td>
										<td><?=$q['size_status']?></td>
										<td>
											<div>
												<button class="btn btn-primary btn-sm" onclick="Update(<?= $q['size_id'] ?>, <?= $q['size_category_id'] ?>, '<?= $q['size_name'] ?>', '<?= $q['size_description'] ?>', '<?= $q['size_status'] ?>')">
													<i class="fa fa-edit"></i> Update
												</button>
												<button class="btn btn-danger btn-sm" onclick="Delete(<?= $q['size_id'] ?>)">
...
<script type="text/javascript">
    ...
    $(() =>
    {
	    ...
	    $('#form').submit(ev =>
	    {
	    	...
	    	if(type == 'ADD')
	    	{
	    		...
	    		$.ajax({
	    			method: 'post',
	    			url: '<?= base_url ?>admin/catalog-size-create',
	    			data: {
	    				size_name: name,
	    				size_category_id: category,
	    	...
	    	else
	    	{
	    		...
	    		$.ajax({
	    			method: 'put',
	    			url: '<?= base_url ?>admin/catalog-size-update',
	    			data: {
	    				size_id: id,
	    				size_name: name,
	    				size_category_id: category,
    	...
    	{ 
	    	...
	    	$.ajax({
    			method: 'delete',
    			url: '<?= base_url ?>admin/catalog-size-delete',
    			data: {
    				size_id: id
    			}
    		})
	...
	// Update button click
    const Update = (id, category, name, description, status) =>
    {
    	type = 'UPDATE'
    	$('#id').val(id)
    	$('#name').val(name)
