						...
						<div class="widget-body">
							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								...
								<tbody>
									...
									<tr data-id="<?= $q['tage_id'] ?>">
										<td><?=$q['tage_name']?></td>
										<td><?=$q['tage_description']?></td>
										<td><?=$q['tage_status']?></td>
										<td>
											<button class="btn btn-primary btn-sm" onclick="Update(<?= $q['tage_id'] ?>, '<?= $q['tage_name'] ?>', '<?= $q['tage_description'] ?>', '<?= $q['tage_status'] ?>')">
												<i class="fa fa-edit"></i> Update
											</button>
											<button class="btn btn-danger btn-sm" onclick="Delete(<?= $q['tage_id'] ?>)">
    ...
    $(() =>
    {
	    ...
	    $('#form').submit(ev =>
	    {
	    	...
	    	if(type == 'ADD')
	    	{
	    		...
	    		$.ajax({
	    			method: 'post',
	    			url: '<?= base_url ?>admin/catalog-tags-create',
	    			data: {
	    				tage_name: name,
	    				tage_description: description,
	    				tage_status: status,
	    			}
	    		})
	    	...
	    	else
	    	{
	    		...
	    		$.ajax({
	    			method: 'put',
	    			url: '<?= base_url ?>admin/catalog-tags-update',
	    			data: {
	    				tage_id: id,
	    				tage_name: name,
	    				tage_description: description,
    	...
    	{ 
	    	...
	    	$.ajax({
    			method: 'delete',
    			url: '<?= base_url ?>admin/catalog-tags-delete',
    			data: {
    				tage_id: id
    			}
    		})
