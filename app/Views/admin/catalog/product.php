							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								<thead>			                
									<tr>
										...
										<th data-hide="phone"><i class="fa fa-fw fa-align-justify text-muted hidden-md hidden-sm hidden-xs"></i> Keywords</th>
										...
										<th data-hide="phone"><i class="fa fa-fw fa-star text-muted hidden-md hidden-sm hidden-xs"></i> Rating</th>
								...
								<tbody>
									...
									<tr data-id="<?= $q['prod_id'] ?>">
										...
										<td>
											<div>
												<button class="btn btn-warning btn-sm" onclick="Detail(<?= $q['prod_id'] ?>, <?= $q['prod_cate_id'] ?>, '<?= $q['prod_name'] ?>', '<?= $q['prod_keyword'] ?>', '<?= $q['prod_url'] ?>', '<?= $q['prod_image'] ?>', '<?= $q['prod_size'] ?>', '<?= $q['prod_tags'] ?>', '<?= $q['prod_color'] ?>', '<?= $q['prod_price'] ?>', '<?= $q['prod_stok'] ?>', '<?= $q['prod_rating'] ?>', `<?= $q['prod_description'] ?>`, '<?= $q['prod_views'] ?>', '<?= $q['prod_upload_by'] ?>', '<?= $q['prod_status'] ?>')">
													<i class="fa fa-eye"></i> Detail
												</button>
												<button class="btn btn-primary btn-sm" onclick="Update(<?= $q['prod_id'] ?>, <?= $q['prod_cate_id'] ?>, '<?= $q['prod_name'] ?>', '<?= $q['prod_keyword'] ?>', '<?= $q['prod_url'] ?>', '<?= $q['prod_image'] ?>', '<?= $q['prod_size'] ?>', '<?= $q['prod_tags'] ?>', '<?= $q['prod_color'] ?>', '<?= $q['prod_price'] ?>', '<?= $q['prod_stok'] ?>', '<?= $q['prod_rating'] ?>', `<?= $q['prod_description'] ?>`, '<?= $q['prod_views'] ?>', '<?= $q['prod_upload_by'] ?>', '<?= $q['prod_status'] ?>')">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								<label for="tanggal">Keywords</label>
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<select multiple class="select2" name="size" id="size" required>
						...
						<div class="col-md-6">
							<div class="form-group">
								<label for="tanggal">Tag</label>
								<select multiple class="select2" name="tag" id="tag" required>
									<?php foreach($tag as $t) : ?>
										<option value="<?= $t['tage_id'] ?>"><?= $t['tage_name'] ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								<label for="tanggal">Color</label>
								<select multiple class="select2" name="color" id="color" required>
...
<script type="text/javascript">
    ...
    // Detail Banner
    const Detail = (id, category, name, keyword, url, image, size, tag, color, price, stok, rating, description, view, upload_by, status) =>
    {
    	type = 'UPDATE'
    	$('#id').val(id)
	...
	// Update button click
    const Update = (id, category, name, keyword, url, image, size, tag, color, price, stok, rating, description, view, upload_by, status) =>
    {
    	type = 'UPDATE'
    	$('#id').val(id)
    	$('#category').val(category)
