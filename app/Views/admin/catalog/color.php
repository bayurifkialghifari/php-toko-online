							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								<thead>			                
									<tr>
										...
										<th data-hide="phone"><i class="fa fa-fw fa-code text-muted hidden-md hidden-sm hidden-xs"></i> Code</th>
										<th data-hide="phone"><i class="fa fa-fw fa-code text-muted hidden-md hidden-sm hidden-xs"></i> Color</th>
								...
								<tbody>
									...
									<tr data-id="<?= $q['color_id'] ?>">
										<td><?=$q['color_name']?></td>
										<td><?=$q['color_code']?></td>
										<td style="background-color: #<?=$q['color_code']?>"></td>
										<td><?=$q['color_description']?></td>
										<td><?=$q['color_status']?></td>
										<td>
											<button class="btn btn-primary btn-sm" onclick="Update(<?= $q['color_id'] ?>, '<?= $q['color_name'] ?>', '<?= $q['color_code'] ?>', '<?= $q['color_description'] ?>', '<?= $q['color_status'] ?>')">
												<i class="fa fa-edit"></i> Update
											</button>
											<button class="btn btn-danger btn-sm" onclick="Delete(<?= $q['color_id'] ?>)">
...
<script type="text/javascript">
    ...
    $(() =>
    {
	    ...
	    $('#form').submit(ev =>
	    {
	    	...
	    	if(type == 'ADD')
	    	{
	    		...
	    		$.ajax({
	    			method: 'post',
	    			url: '<?= base_url ?>admin/catalog-color-create',
	    			data: {
	    				color_name: name,
	    				color_code: code,
	    	...
	    	else
	    	{
	    		...
	    		$.ajax({
	    			method: 'put',
	    			url: '<?= base_url ?>admin/catalog-color-update',
	    			data: {
	    				color_id: id,
	    				color_name: name,
	    				color_code: code,
    	...
    	{ 
	    	...
	    	$.ajax({
    			method: 'delete',
    			url: '<?= base_url ?>admin/catalog-color-delete',
    			data: {
    				color_id: id
    			}
    		})
	...
	// Update button click
    const Update = (id, name, code, description, status) =>
    {
    	type = 'UPDATE'
    	$('#id').val(id)
    	$('#name').val(name)
