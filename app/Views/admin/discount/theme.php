						...
						<div class="widget-body">
							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								...
								<tbody>
									...
									<tr data-id="<?= $q['dist_id'] ?>">
										<td><?=$q['dist_name']?></td>
										<td><?=$q['dist_description']?></td>
										<td><?=$q['dist_status']?></td>
										<td>
											<button class="btn btn-primary btn-sm" onclick="Update(<?= $q['dist_id'] ?>, '<?= $q['dist_name'] ?>', '<?= $q['dist_description'] ?>', '<?= $q['dist_status'] ?>')">
												<i class="fa fa-edit"></i> Update
											</button>
											<button class="btn btn-danger btn-sm" onclick="Delete(<?= $q['dist_id'] ?>)">
    ...
    $(() =>
    {
	    ...
	    $('#form').submit(ev =>
	    {
	    	...
	    	if(type == 'ADD')
	    	{
	    		...
	    		$.ajax({
	    			method: 'post',
	    			url: '<?= base_url ?>admin/discount-theme-create',
	    			data: {
	    				dist_name: name,
	    	...
	    	else
	    	{
	    		...
	    		$.ajax({
	    			method: 'put',
	    			url: '<?= base_url ?>admin/discount-theme-update',
	    			data: {
	    				dist_id: id,
	    				dist_name: name,
	    				dist_description: description,
    	...
    	{ 
	    	...
	    	$.ajax({
    			method: 'delete',
    			url: '<?= base_url ?>admin/discount-theme-delete',
    			data: {
    				dist_id: id
    			}
    		})
